...
export default function AdminProdutos() {
  ...
  const importCSV = async e => {
    ...
    try{const imported=parseShopeeCSV(await file.text());if(imported.length>50)throw Error('Este painel aceita até 50 produtos por arquivo. Divida o CSV em lotes.');
  ...
  return <main className="min-h-screen overflow-y-auto bg-slate-50 px-4 py-8 text-slate-900 sm:px-8" style={{height:'100dvh',backgroundColor:'#f8fafc',color:'#0f172a',fontFamily:'Arial, sans-serif'}}>
    <div className="mx-auto max-w-6xl space-y-6">
      <header className="space-y-2"><a href="/" className="text-sm text-emerald-700 hover:underline">← Voltar à vitrine</a><p className="text-xs font-bold uppercase tracking-widest text-emerald-700">Utilidades Essenciais · Administração</p><h1 className="text-3xl font-bold sm:text-4xl">Cadastro de produtos</h1><p className="text-slate-600">Cadastre até 50 produtos por lote. O painel verifica duplicatas antes de inserir na sua planilha.</p></header>
      <section className="rounded-2xl border border-slate-200 bg-white p-5"><label className="mb-2 block text-sm font-semibold" htmlFor="password">Senha administrativa</label><div className="flex flex-col gap-3 sm:flex-row"><input id="password" type="password" autoComplete="off" value={password} onChange={e=>setPassword(e.target.value)} className="w-full rounded-lg border border-slate-300 bg-white p-3 outline-none focus:border-emerald-500" placeholder="Senha configurada na Vercel"/><button type="button" disabled={loading || !password} onClick={connect} className="rounded-lg bg-emerald-600 px-5 py-3 font-semibold text-white disabled:opacity-40">Conectar</button></div>{summary && <p className="mt-3 text-sm text-emerald-700">Conectado à planilha · {summary.total} produtos ativos</p>}</section>
      ...
      <form onSubmit={submit} className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-semibold">Produtos para cadastrar ({rows.length}/50)</h2><button type="button" disabled={rows.length>=50} onClick={()=>setRows(prev=>[...prev,blank()])} className="rounded-lg border border-slate-400 px-4 py-2 text-sm hover:border-lime-300 disabled:opacity-40">+ Adicionar produto</button></div>
