...
export default function AdminProdutos() {
  ...
  return <main className="min-h-screen overflow-y-auto bg-slate-950 px-4 py-8 text-slate-100 sm:px-8" style={{height:'100dvh'}}>
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      {[[ 'name','Nome do produto *','text'],['affiliateLink','Link de afiliado *','url'],['videoUrl','URL pública do vídeo (opcional)','url'],['price','Preço atual (R$)','text'],['originalPrice','Preço anterior real (R$)','text'],['imageUrl','URL da imagem','url']].map(([key,label,type])=><label key={key} className="block text-sm"><span className="mb-1 block text-slate-300">{label}</span><input type={type} value={r[key]} onChange={e=>change(i,key,e.target.value)} placeholder={key==='price'?'39,90':undefined} className="w-full rounded-lg border border-slate-600 bg-slate-950 p-3 outline-none focus:border-lime-400" /></label>)}
