...
export default function AdminProdutos() {
  ...
  const connect = async () => { setLoading(true);setFeedback(null);try {setSummary(await call('list'));setShowPassword(false);} catch(e){setFeedback({error:e.message});}finally{setLoading(false);} };
  ...
  return <main className="min-h-screen overflow-y-auto bg-slate-50 px-4 py-8 text-slate-900 sm:px-8" style={{height:'100dvh',backgroundColor:'#f8fafc',color:'#0f172a',fontFamily:'Arial, sans-serif'}}>
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <label className="block text-sm"><span className="mb-1 block text-slate-700">Categoria</span><select value={r.category} onChange={e=>change(i,'category',e.target.value)} className="w-full rounded-lg border border-slate-300 bg-white p-3">{categories.map(c=><option key={c}>{c}</option>)}</select></label>
      </div></section>)}
