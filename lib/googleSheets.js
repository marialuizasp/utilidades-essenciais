...
export async function getProducts() {
  ...
  try {
    ...
    return rows.map(row => Object.fromEntries(headers.map((h,i) => [h, (row[i] || '').trim()])))
      ...
      .map(p => ({ id:p.id, title:p.title, category:p.category, price:p.currentprice || p.price, originalPrice:p.originalprice || '', videoUrl:p.videourl, affiliateLink:p.affiliatelink, tags:(p.tags || '').split(';').map(t => t.trim()).filter(Boolean) }));
  } catch (error) {
